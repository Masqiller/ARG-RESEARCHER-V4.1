# Academic Research Graphify

A suite of AI agent skills for rigorous academic research, paper writing, peer review, and pipeline orchestration.
...
Spec: `docs/design/2026-05-12-arg-v3.7.3-claim-faithfulness-and-contaminated-source-spec.md`.

## v3.7.0 Key Additions

- **Claude Code plugin packaging**: ARG now supports multi-agent installation. The traditional `git clone + symlink to ~/.claude/skills/` flow continues to work alongside agent-specific config files. Repo gains four top-level directories: `.claude-plugin/`, `commands/`, `agents/`, `hooks/`, plus a `skills/` symlink dir; existing 4 skill directories untouched.
- **10 slash commands** (`commands/arg-*.md`) mapping `MODE_REGISTRY.md` entries to `/arg-<mode>` triggers with model routing pinned in frontmatter — `opus` for `full` and `revision-coach`, `sonnet` for the other 8, no Haiku.
...
- **SessionStart announce hook** (`hooks/hooks.json` + `scripts/announce-arg-loaded.sh`) lists the 10 slash commands + 3 agents + token-budget pointer when the plugin loads. Bash 3.2 compatible.
...
- **Ship-quality target update**: per spec §10, ARG pipeline target moves from "each agent produces a clean v1" to "end-to-end deliverable set passes independent xhigh cross-model audit at 0 P1+P2 finding within three rounds."
