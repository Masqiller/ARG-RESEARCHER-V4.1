# ARG v3.6.2 — Sprint Contract Design

**Date**: 2026-04-23
**Target release**: v3.6.2 (sprint contract, reviewer-only first test case)
**Roadmap anchor**: `ARG-Researcher-ROADMAP.md` §3.6.2
...
Format: `domain/mode/version`. The version here is the *template* version (bumped whenever the template adds, removes, or changes a dimension or failure condition). It is intentionally decoupled from ARG semver; template revisions do not force a suite release.
...
The ARG release under which this contract template was authored. Used by `check_sprint_contract.py` for SC-1 baseline-lag warning (§4.2).
...
- **SC-1 baseline lag**. If `--arg-version` provided and `baseline_version` lags current ARG by more than 2 minor versions, warn: `WARNING: contract baseline lags ARG by N minor; retirement candidate`. If `--arg-version` not provided, SC-1 is skipped (no implicit filesystem reads).
...
```
python scripts/check_sprint_contract.py <contract.json> [--arg-version vX.Y.Z]
```
...
### 4.5 ARG version source

The validator does not assume a version-file location. The caller passes `--arg-version` explicitly. In CI, the version is extracted from the top `## [x.y.z]` heading in `CHANGELOG.md` (see §8 for the shell snippet).
...
- `test_shipped_templates_produce_zero_soft_warnings` — load both templates, run `warn_suspicious()` with `--arg-version v3.6.2`, assert the returned list is empty. Rationale: shipped templates must be clean; a warning there indicates either template bug or validator regression.
...
- `test_sc1_no_ars_version_skips` — no `--arg-version` means SC-1 not printed even with old baseline.
...
- name: Validate sprint contract templates
  run: |
    ...
    for f in shared/contracts/**/*.json; do
      python scripts/check_sprint_contract.py "$f" --arg-version "v${ARS_VERSION}"
    done
```
...
| 2026-04-23 | Validator accepts `--arg-version` via CLI, no filesystem assumption | Pattern-match `check_compliance_report.py` structure while avoiding hidden coupling to any specific version file. |
| 2026-04-23 | Template versioning decoupled from ARG semver | `contract_id` carries template version `v1/v2/...`; this is independent of ARG `v3.6.2`. Aligns with `feedback_spec_location_semver_decoupling.md`. |
...
- ROADMAP: `~/Projects/claude-memory-sync/花花/roadmaps/ARG-Researcher-ROADMAP.md` §3.6.2
