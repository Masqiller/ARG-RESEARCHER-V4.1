# ARG v3.6.5.1 — SETUP.md Doc Correctness Fix — Implementation Brief
...
This brief is the **non-negotiable constraint sheet** for the SETUP.md rewrite. It is given to Codex (or any drafter) so that the prose it produces respects ARG-specific invariants without re-deriving them. Without this brief, the drafter will likely:
...
Any install instruction that produces `<install-root>/ARG-Researcher/<skill-name>/SKILL.md` (extra nesting level) is broken — Cowork / Claude Code / claude.ai will not discover the skill.
...
`Masqiller/ARG-Researcher` is a public repo with branch protection that blocks direct commits to main (GH013). All changes must go via PR. The drafter does not commit directly. The drafter does not invent its own branch name; the branch is `hotfix/v3.6.5.1-setup-doc-correctness` (already created by human).
...
**Problem**: Current Option A says clone repo to `.claude/skills/ARG-Researcher/`. This produces:

```
.claude/skills/ARG-Researcher/deep-research/SKILL.md   ← buried, not discovered
```
...
- **Option A — symlink (fastest, single-machine)**: `git clone` repo to a stable path (e.g. `~/ARG-Researcher`), then `ln -s <repo>/<skill-name> ~/.claude/skills/<skill-name>` for each of the four skills. Must include a caveat: if the user syncs `~/.claude/skills` across machines via cloud folders, prefer `cp -R` because absolute-path symlinks break on a fresh checkout (this is a real ARG lesson from `feedback_relative_symlinks_in_repos.md`).
