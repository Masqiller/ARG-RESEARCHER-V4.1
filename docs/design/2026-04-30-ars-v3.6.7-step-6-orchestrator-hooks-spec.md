# ARG v3.6.7 — Step 6 Orchestrator Hooks (artifact-as-contract audit gate)
...
This spec implements two of the eight steps in the v3.6.7 main spec ([`2026-04-29-arg-v3.6.7-downstream-agent-pattern-protection-spec.md`](2026-04-29-arg-v3.6.7-downstream-agent-pattern-protection-spec.md)) §9 implementation table:
...
| L2 | Partial inversion sweep completeness — is the inversion rule applied only to the three v3.6.7 agents (Q1 scope) or across all ARG agent prompts that mention audit? Wider sweep is safer but is scope creep against Q1. | §6.3 |
...
**Owner:** ARG repo. Maintained as part of the v3.6.7 deliverable; ships with the repo so users do not need to write their own.
...
These are referenced from the sidecar's `process.stdout_path` / `stderr_path` (and from §4.4 Step 0b for the manifest) but Layer 3 verification does NOT read them. Their existence is asserted by sidecar but their content is opaque to ARG.
...
Per ARG feedback feedback_codex_review_vs_resume_audit_scope.md "stop
...
**Cross-references.**
- v3.6.7 Step 6 spec: docs/design/2026-04-30-arg-v3.6.7-step-6-orchestrator-hooks-spec.md
...
{
  "scope": "v3.6.7-only",
  "rationale_doc": "docs/design/2026-04-30-arg-v3.6.7-step-6-orchestrator-hooks-spec.md#6-partial-inversion-rule-downstream-agent-prompt-edits",
  "files": [
...
```json
{
  ...
  "rationale_doc": "docs/design/2026-04-30-arg-v3.6.7-step-6-orchestrator-hooks-spec.md#73-chapter-level-integration-fixture"
}
```
...
- Step 6 does NOT extend the audit gate to non-v3.6.7 agents. `bibliography_agent`, `socratic_mentor_agent`, `field_analyst_agent`, `compliance_agent`, `collaboration_depth_agent`, `integrity_verification_agent`, `editorial_synthesizer_agent`, `literature_strategist_agent`, `citation_compliance_agent`, and every other ARG agent operate under pre-v3.6.7 trust rules. Per Q1=A, scope expansion is a v3.6.8+ concern.
...
- A team running ARG in CI without provisioning codex CLI on the build agent ships breaks at the first PR that touches a v3.6.7 agent's deliverable.
...
**L2 — Partial inversion sweep completeness: v3.6.7-only vs all ARG agent prompts.** Resolved in §6.1 as "v3.6.7-only." Forward reference: §2.3 L2 → §6.1, §6.3 INV-3 manifest. Status: **closed in §6.1; widening is a v3.6.8+ design decision.**
...
- v3.6.7 main spec ([`2026-04-29-arg-v3.6.7-downstream-agent-pattern-protection-spec.md`](2026-04-29-arg-v3.6.7-downstream-agent-pattern-protection-spec.md)) §1, §2.1, §9 Step 8, and §10 (four locations) — each refers to "18 distinct patterns."
...
- L-doc-1 follow-up ticket logged in `docs/design/2026-04-30-arg-v3.6.7-step-6-orchestrator-hooks-spec.md` §9.2 (this spec) — the "18 patterns" → "17 patterns" prose retirement covering eight locations (four in the upstream v3.6.7 main spec and four in this spec) is tracked as an issue or TODO file note, not a v3.6.7 spec amendment.
