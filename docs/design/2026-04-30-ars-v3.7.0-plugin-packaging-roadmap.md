# ARG v3.7.0 Plugin Packaging Roadmap
...
2. **Contract gap.** `run_codex_audit.sh` requires `--stage <1-6> --agent <name> --deliverable <repo-relative-path>`. The `SubagentStop` hook payload only carries `agent_id`, `agent_type`, `session_id`, `cwd`, `transcript_path`. There is no documented propagation path from ARG pipeline state (current stage + which deliverable just landed) into the hook payload. Building a hook that half-infers stage/deliverable would be design lying — either the wrapper runs against the wrong file, or the hook degrades to "remind the user to invoke the wrapper externally," which is no better than documenting the wrapper's CLI in the README.

Decision: **Phase 2.2 ships only the SessionStart announce hook.** A real SubagentStop → `run_codex_audit.sh` integration is deferred to v3.6.8+ when ARG gains a stage/deliverable propagation contract (likely as a passport ledger entry that the hook can read from `${CLAUDE_PROJECT_DIR}` or a fixed location under `audit_artifacts/`). The sections below describing Step 1–3 of the original Task 2.4 are retained as historical context but treated as obsoleted reference; the actual Phase 2.2 implementation lives at `hooks/hooks.json` and `scripts/announce-arg-loaded.sh`.

The `feedback_no_haiku.md` model-floor concern that motivated PostToolUse-based audit triggering is partially addressed at a different layer: ARG slash commands pin `opus`/`sonnet` in their frontmatter, plugin agents use `model: inherit`, and the user's PreToolUse `warn-agent-no-model.sh` hook enforces no-haiku at Agent dispatch. The remaining gap is "did codex actually audit the deliverable?" — that gate stays human-driven (CI workflow step or external terminal) for v3.7.0.

---

**Goal:** 將 ARG 從「git clone + symlink 到 `~/.claude/skills/`」的分發模式，升級成 Claude Code Plugin（`/plugin install` 一行裝），同時保留 GitHub repo 作為 source of truth 與教學內容主入口。
...
- ❌ Codex CLI：用 `ARG-Researcher-codex` fork，不在本 roadmap 範圍
...
**目標：** 最小工程量讓使用者可以 `/plugin marketplace add Masqiller/ARG-Researcher` + `/plugin install ars`，4 個既有 skill 自動被載入。
...
```json
{
  "name": "ARG-Researcher",
  "version": "3.7.0",
  "description": "Academic Research Graphify for Claude Code: research → write → review → revise → finalize. 4 skills + 10 modes + Material Passport pipeline.",
  "author": {
    "name": "See CONTRIBUTORS.md"
  },
  "homepage": "https://github.com/Masqiller/ARG-Researcher",
  "repository": "https://github.com/Masqiller/ARG-Researcher",
  "license": "Other",
...
{
  "name": "ARG-Researcher",
  "displayName": "Academic Research Graphify",
...
建議在 `/plugin` UI 開啟 auto-update，這樣 ARG 每次發新版會自動拉到本地。
...
Run: `cd /tmp && claude --plugin-dir ~/Projects/ARG-Researcher`
...
**驗收：** 使用者在 session 跑 `/arg-full`、`/arg-revision`、`/arg-lit-review` 等 slash 指令直接觸發對應 mode；agent council 派工自動走 sonnet/opus 不掉到 haiku。

### Task 2.1: 列出 10 個 mode 對映表

**Files:**
- Reference: `MODE_REGISTRY.md`
- Create: `docs/design/2026-04-30-arg-mode-to-command-mapping.md`

- [ ] **Step 1: 從 MODE_REGISTRY.md 抽 10 mode + 寫對映**

| Mode | Slash command | 觸發條件 | 預設 model |
|---|---|---|---|
| full | `/arg-full` | 完整 pipeline | opus（架構決策）|
| plan | `/arg-plan` | 只規劃不寫 | sonnet |
| outline | `/arg-outline` | 大綱 | sonnet |
| revision | `/arg-revision` | 改稿 | sonnet |
| revision-coach | `/arg-revision-coach` | 改稿教練 | opus |
| abstract | `/arg-abstract` | 摘要 | sonnet |
| lit-review | `/arg-lit-review` | 文獻回顧 | sonnet |
| format-convert | `/arg-format-convert` | 格式轉換 | sonnet |
| citation-check | `/arg-citation-check` | 引用檢查 | sonnet |
| disclosure | `/arg-disclosure` | AI disclosure | sonnet |
...
**Files:**
- Create: `commands/arg-full.md` 至 `commands/arg-disclosure.md`（10 個檔）
...
Trigger ARG full mode. Load academic-pipeline skill and execute the complete academic research workflow.

ARG spec: docs/design/2026-04-29-arg-v3.6.7-downstream-agent-pattern-protection-spec.md
...
```bash
git add commands/
git commit -m "feat(v3.7.0): add 10 slash commands for ARG modes

Maps MODE_REGISTRY entries to /arg-* slash commands with explicit model
...
- [ ] **Step 1: 從既有 ARG spec 抽 agent 定義 → frontmatter**
...
description: ARG bibliography agent — generates citations with hallucination guards
model: sonnet
...
```json
{
  "hooks": {
    ...
    "SessionStart": [
      {
        "command": "scripts/announce-arg-loaded.sh",
        "description": "Announce ARG plugin loaded with active mode hint"
      }
    ]
  }
}
...
```bash
git add hooks/ scripts/codex-audit-on-phase-complete.sh scripts/announce-arg-loaded.sh
...
```bash
cd /tmp && git clone ~/Projects/ARG-Researcher arg-plugin-test
cd arg-plugin-test
claude --plugin-dir .
# In session: /arg-lit-review "test topic"
```
...
**驗收：** 在另一台機器 `/plugin marketplace add Masqiller/ARG-Researcher` 後開 auto-update，repo push 新 commit 後重啟 Claude Code 自動拉新版。

### Task 3.1: 確認 marketplace 自動更新行為

**重要事實（claude-code-guide 已查證；2026-05-05 codex review F-004 校正）：**
- 第三方 plugin（如 ARG）預設 **auto-update OFF**
- 使用者要在 `/plugin` UI 切 marketplace 為 auto-update on
- 手動更新已安裝 plugin：`/plugin update ARG-Researcher`
- 重新拉 marketplace 來源：`/plugin marketplace update ARG-Researcher`（這個只 refresh 來源清單，不會更新已裝 plugin）
...
```markdown
> **強烈建議開啟 auto-update：** 進 `/plugin` UI → 找到 ARG-Researcher → 開 auto-update。
> ARG 每 1-2 週發新版，開了 auto-update 才能自動同步。
> 沒開 auto-update 的話：手動跑 `/plugin update ARG-Researcher`。
```
...
│   ├── arg-outline.md
│   ├── arg-revision.md
│   ├── arg-revision-coach.md
│   ├── arg-abstract.md
│   ├── arg-lit-review.md
│   ├── arg-format-convert.md
│   ├── arg-citation-check.md
│   └── arg-disclosure.md
...
├── scripts/
│   └── announce-arg-loaded.sh              [Phase 2.2 新增 — Bash 3.2 compatible]
...
| Codex CLI 用戶誤裝 plugin | 低 | README 明寫「plugin = Claude Code only；Codex 用 ARG-Researcher-codex」 |

---

## 與另一個 ARG Session 的隔離
...
- `agents/`
- `hooks/`
- `docs/design/2026-04-30-arg-v3.7.0-plugin-packaging-roadmap.md`（本檔）
...
- 與另一個 session 的隔離 → 專章「與另一個 ARG Session 的隔離」
...
**3. Type consistency：** plugin.json `name` = "ARG-Researcher"、marketplace.json `name` 同字串、skill 目錄名 deep-research / academic-paper / academic-paper-reviewer / academic-pipeline 全程一致。Slash command 命名 `/arg-*` 一致。

---

## Execution Handoff

**Plan complete and saved to `docs/design/2026-04-30-arg-v3.7.0-plugin-packaging-roadmap.md`. 三個執行選項：**
