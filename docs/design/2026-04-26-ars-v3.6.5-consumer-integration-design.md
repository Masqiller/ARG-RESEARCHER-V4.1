# ARG v3.6.5 — Material Passport `literature_corpus[]` Consumer Integration
...
**Repository:** Masqiller/ARG-Researcher
...
1. **Inclusion / Exclusion parity.** The lint cannot enforce that an LLM applies the same criteria to corpus and external sources. Agent files use BAD / GOOD example pairs (existing ARG anti-pattern teaching style) to make the parity rule load-bearing in prose.
