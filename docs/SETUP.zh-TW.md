...
Academic Research Graphify 的前置需求與選用設定。只需要 Markdown 輸出與預設 Claude Opus 4.7 pipeline 的人，大部分內容可以略過。請見下方「最小可行設定」。
...
3. 在這個 repo（或任何把 ARG 放在 `.claude/skills/` 下的專案）執行 `claude`。
...
如果你已經維護一個策展過的文獻語料（Zotero、Obsidian、PDF 資料夾等），可以先把它打包進 Material Passport，讓 Phase 1 ARG agent 在去外部資料庫搜尋之前先讀你的文獻庫。此功能採 opt-in 與 presence-based 設計。沒提供語料時，ARG 走 external-DB-only flow，行為不變。
...
ARG 暴露若干 opt-in flag，全部預設 OFF；設定後僅影響當前 session。
...
ARG 使用 Claude Opus 4.7 即可完整運作。想要更高信心，可選擇啟用第二 AI 模型來獨立驗證完整性檢查，並挑戰魔鬼代言人。
...
不要把整個 repository 當成單一巢狀 skill 資料夾安裝到 `.claude/skills/ARG-Researcher/`。那會讓四個 `SKILL.md` 比 Claude 可發現的位置多埋一層。請參考 Anthropic 的 [Claude Code Skills documentation](https://code.claude.com/docs/en/skills)。

### 方法零：Claude Code Plugin（v3.7.0+，Claude Code CLI / IDE 用戶推薦）

如果你用的是 Claude Code CLI、VS Code extension 或 JetBrains extension，可以一行指令安裝 ARG：
...
**強烈建議開啟 auto-update。** 進 `/plugin` UI 找到 `ARG-Researcher`，把 auto-update 開起來。ARG 大約 1–2 週發新版，開了之後會自動同步。手動更新已安裝的 plugin：`/plugin update ARG-Researcher`。（`/plugin marketplace update ARG-Researcher` 只重新拉 marketplace 來源，不會更新已裝 plugin。）
...
- ➡️ Codex CLI — 改裝姊妹版 [`Masqiller/ARG-Researcher-codex`](https://github.com/Masqiller/ARG-Researcher-codex)（同一套 workflow 內容、Codex 原生包裝）

### 方法一：作為專案 Skills（推薦）

當你希望 ARG 可在既有 Claude Code 專案內使用時，請用此方式。
...
當你想直接在 ARG repository 內工作時，請用此方式。
...
1. 前往 <https://github.com/Masqiller/ARG-Researcher>
...
當你想在 [Claude Cowork](https://support.claude.com/en/articles/13345190-get-started-with-claude-cowork) 使用四個 ARG skills 時，請用此方式。Cowork 是 Claude Desktop 的 agentic workspace。
...
ARG 是為 Claude Code 設計的 skill suite。四個 skill 各自是 12-13 個 agent 組成的工作團隊，仰賴多 agent 協作、`scripts/` 下可執行的轉接器，以及 Material Passport 的檔案交接。claude.ai 網頁版的執行環境跟 Claude Code 不同，要把這個 repository 接進 claude.ai 有兩條路徑，差別很大：
...
3. 從 GitHub 匯入：在 Project 中，點擊 **Files** → **+** → **GitHub** → 選擇 `Masqiller/ARG-Researcher`。
4. 選取以下資料夾與檔案。
...
每個 zip 都會被 upload UI 以 description 過長拒絕，因為 ARG 所有 description 都超過 claude.ai 200 字元上限。Description 維持原狀並非疏忽，原因見上方說明。
