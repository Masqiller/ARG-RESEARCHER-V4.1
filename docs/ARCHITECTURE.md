# ARG Pipeline Architecture (v3.7.0)
...
The Material Passport's `literature_corpus[]` is an **optional** Schema 9 input port for user-curated literature. Producers (out-of-band, before an ARG session) and consumers (Phase 1 literature agents at runtime) sit on opposite sides of the passport.
...
flowchart LR
    ...
    Adapter --> Passport
    subgraph Consumer["Phase 1 ARG runtime (v3.6.5)"]
...
Authoritative references: [`academic-pipeline/references/literature_corpus_consumers.md`](../academic-pipeline/references/literature_corpus_consumers.md) (consumer protocol) + [`academic-pipeline/references/adapters/overview.md`](../academic-pipeline/references/adapters/overview.md) (adapter contract) + [`docs/design/2026-04-26-arg-v3.6.5-consumer-integration-design.md`](design/2026-04-26-arg-v3.6.5-consumer-integration-design.md) (consumer design).
...
## 8. ARG Evolution Timeline

```mermaid
timeline
    title ARG evolution timeline
    ...
    v3.7.0 : Claude Code plugin packaging
           ...
           : 10 slash commands (commands/arg-*.md, model pinned opus/sonnet, no haiku)
           : 3 plugin agents (agents/, symlink to v3.6.7-hardened source, model: inherit)
           : SessionStart announce hook (hooks/hooks.json + announce-arg-loaded.sh)
```
