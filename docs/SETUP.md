...
Prerequisites and optional setup for Academic Research Graphify. If you only need Markdown output and the default Claude Opus 4.7 pipeline, you can skip most of this — see "Minimum viable setup" below.
...
3. `claude` in this repo (or any project that has ARG in `.claude/skills/`).
...
# 3. Pass the resulting passport.yaml into your ARG session
...
ARG exposes a few opt-in flags. All default to OFF; setting them changes behaviour for the current session only.
...
ARG works with Claude Opus 4.7 alone. For higher confidence, you can optionally enable a second AI model to independently verify integrity checks and challenge the devil's advocate.
...
Do not install the whole repository as one nested skill folder under `.claude/skills/ARG-Researcher/`; that buries the four `SKILL.md` files one level too deep for discovery. See Anthropic's [Claude Code Skills documentation](https://code.claude.com/docs/en/skills).

### Method 0: Claude Code Plugin (v3.7.0+, recommended for Claude Code CLI / IDE users)

If you use Claude Code CLI, VS Code extension, or JetBrains extension, install ARG as a plugin:

```text
/plugin marketplace add Masqiller/ARG-Researcher
...
**Strongly recommended: open auto-update.** Open the `/plugin` UI, find `ARG-Researcher`, and toggle auto-update on. ARG releases roughly every 1–2 weeks; auto-update keeps you in sync without manual refreshes. To refresh manually: `/plugin update ARG-Researcher`. (`/plugin marketplace update ARG-Researcher` only refreshes the marketplace source list, not the installed plugin itself.)
...
- ➡️ Codex CLI — install the sibling distribution [`Masqiller/ARG-Researcher-codex`](https://github.com/Masqiller/ARG-Researcher-codex) (same workflow content, Codex-native packaging)

### Method 1: As project skills (recommended)

Use this when you want ARG available inside an existing Claude Code project.
...
cp -R ~/ARG-Researcher/deep-research .claude/skills/deep-research
cp -R ~/ARG-Researcher/academic-paper .claude/skills/academic-paper
cp -R ~/ARG-Researcher/academic-paper-reviewer .claude/skills/academic-paper-reviewer
cp -R ~/ARG-Researcher/academic-pipeline .claude/skills/academic-pipeline
```
...
Use this when you want to work directly inside the ARG repository.

```bash
git clone https://github.com/Masqiller/ARG-Researcher.git
cd ARG-Researcher
claude
```
...
1. Go to <https://github.com/Masqiller/ARG-Researcher>
...
Use this when you want the four ARG skills available in [Claude Cowork](https://support.claude.com/en/articles/13345190-get-started-with-claude-cowork), Claude Desktop's agentic workspace.
...
mkdir -p ~/.claude/skills
cd ~/.claude/skills
ln -s ~/ARG-Researcher/deep-research deep-research
ln -s ~/ARG-Researcher/academic-paper academic-paper
ln -s ~/ARG-Researcher/academic-paper-reviewer academic-paper-reviewer
ln -s ~/ARG-Researcher/academic-pipeline academic-pipeline
```
...
ARG is a Claude Code-native suite. The four skills are 12-13-agent teams that depend on multi-agent orchestration, executable scripts under `scripts/`, and Material Passport file handoffs. claude.ai's web interface delivers a different runtime than Claude Code, and the two access paths it offers reach this repository in different ways:
...
3. Import from GitHub: in the Project, click **Files** → **+** → **GitHub** → select `Masqiller/ARG-Researcher`.
...
The upload UI will reject each zip with a description-too-long error because every ARG description exceeds claude.ai's 200-character cap. The descriptions are intentionally not trimmed; see the rationale above.
