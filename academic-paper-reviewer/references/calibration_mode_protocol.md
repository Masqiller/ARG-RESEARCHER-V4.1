...
Translation for ARG: **our reviewer has an error profile too, and we do not currently measure it.** Calibration mode closes that gap. It does not try to make the reviewer perfect; it makes the reviewer's imperfections legible.
...
2. **Fresh context per run**: Lu 2026 allowed within-session memory across runs. ARG uses fresh context to prevent cascading errors from a single run's misreading.
...
- **Activation**: opt-in only. User invokes `calibration` mode explicitly. ARG does not auto-calibrate on first use in a new domain.
...
- ARG `shared/cross_model_verification.md` — cross-model reviewer integration.
- ARG `academic-paper-reviewer/references/quality_rubrics.md` — scoring rubric definitions.
