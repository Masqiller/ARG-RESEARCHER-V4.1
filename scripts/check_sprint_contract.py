#!/usr/bin/env python3
"""Validate an ARG sprint contract against sprint_contract.schema.json.

Usage: python scripts/check_sprint_contract.py path/to/contract.json [--arg-version vX.Y.Z]
...
# v? accepts both 'v3.6.2' and '3.6.2' on --arg-version CLI input;
...
def warn_suspicious(contract: dict, ars_current_version: str | None) -> list[str]:
    ...
    # SC-1 baseline lag: contract.baseline_version lags current ARG by > 2 minor
    ...
    if bv and cv:
        ...
        if bv_major == cv_major and (cv_minor - bv_minor) > 2:
            warnings.append(
                ...
                f"current ARG v{cv_major}.{cv_minor}.* by {cv_minor - bv_minor} minor; "
                "retirement candidate"
            )
        elif bv_major != cv_major:
            warnings.append(
                f"SC-1 WARNING: contract baseline major v{bv_major} differs from "
                f"current ARG v{cv_major}; retirement candidate"
            )
...
def main() -> int:
    ...
    parser.add_argument("--arg-version", type=str, default=None,
                        help="Current ARG version (e.g. v3.6.2) for SC-1 baseline lag check")
