#!/usr/bin/env python3
"""ARG v3.7.1 byte-equivalence SHA gate + Step 3a Two-Layer Citation Emission invariants.

Spec: docs/design/2026-04-30-arg-v3.6.8-trust-provenance-and-drift-transparency-spec.md
...
def _resolve_default_branch() -> tuple[str | None, str | None]:
    ...
    return None, (
        "[ARG-V3.7.1 LINT ERROR: default branch unresolvable; clone must "
...
def _ensure_full_clone() -> str | None:
    ...
    if rc != 0:
        return f"[ARG-V3.7.1 LINT ERROR: cannot determine clone depth: {out!r}]"
    ...
    if rc != 0:
        return (
            "[ARG-V3.7.1 LINT ERROR: shallow clone detected; set fetch-depth: "
...
def _v3_6_7_base_commit() -> tuple[str | None, str | None]:
    ...
    if rc != 0 or not out:
        return None, (
            "[ARG-V3.7.1 LINT ERROR: cannot derive v3.6.7 base commit "
...
def _v3_6_7_manifest_unchanged_in_pr() -> tuple[bool, str | None]:
    ...
    if rc_log != 0:
        ...
        return False, (
            "[ARG-V3.7.1 LINT ERROR: anti-self-baseline guard cannot list "
    ...
    if touching:
        ...
        return False, (
            "[ARG-V3.7.1 LINT ERROR: anti-self-baseline guard tripped: "
...
def _read_blob_at_commit(commit: str, repo_relpath: str) -> tuple[bytes | None, str | None]:
    ...
    if result.returncode != 0:
        ...
        return None, (
            f"[ARG-V3.7.1 LINT ERROR: `git show {commit}:{repo_relpath}` "
            f"failed: {stderr!r}]"
        )
...
def _load_v3_6_8_manifest() -> tuple[dict | None, str | None]:
    ...
    if not V3_6_8_MANIFEST.exists():
        return None, (
            "[ARG-V3.7.1 LINT ERROR: v3.6.8 manifest missing at "
            f"{V3_6_8_MANIFEST.relative_to(REPO_ROOT)}]"
        )
    try:
        data = json.loads(V3_6_8_MANIFEST.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError) as exc:
        return None, f"[ARG-V3.7.1 LINT ERROR: v3.6.8 manifest unreadable: {exc}]"
    if data.get("scope") != "v3.6.8-only":
        return None, (
            "[ARG-V3.7.1 LINT ERROR: v3.6.8 manifest 'scope' must be "
            f"'v3.6.8-only', got {data.get('scope')!r}]"
        )
    files = data.get("files")
    if not isinstance(files, list) or not all(isinstance(p, str) for p in files):
        return None, (
            "[ARG-V3.7.1 LINT ERROR: v3.6.8 manifest 'files' must be a list "
...
def check_step3a_invariants(verbose: bool = True) -> int:
    ...
    if failures:
        print("[ARG-V3.7.1 LINT ERROR: Step 3a Two-Layer Citation Emission invariants]")
...
def check_byte_equivalence(verbose: bool = True) -> int:
    ...
    if not files_v367:
        ...
        print(
            "[ARG-V3.7.1 LINT ERROR: v3.6.7 manifest carries empty file list; "
    ...
    if failures:
        print("[ARG-V3.7.1 LINT ERROR: v3.6.7 PATTERN PROTECTION block byte-equivalence failures]")
        for line in failures:
            print(line)
        return 1
    if verbose:
