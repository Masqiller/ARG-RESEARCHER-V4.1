#!/usr/bin/env python3
"""ARG v3.7.1 trust-chain frontmatter lint (spec § 3.1 firm rules).
...
def _load_yaml_or_json(path: Path) -> Any:
    ...
    try:
        ...
        raise SystemExit(f"[ARG-V3.7.1 LINT ERROR: cannot parse {path}: {exc}]")
...
def check_payload(payload: Any, source: str) -> list[str]:
    ...
    for idx, entry in enumerate(_iter_entries(payload)):
        ...
        for err in check_entry(entry, label):
            failures.append(err)
    if failures:
        failures.insert(0, f"[ARG-V3.7.1 LINT ERROR: trust-chain rule violations in {source}]")
    return failures
...
def _scan_examples() -> int:
    ...
    if not EXAMPLES_ROOT.exists():
        print(f"[ARG-V3.7.1 LINT ERROR: examples root missing at {EXAMPLES_ROOT}]")
        return 1
    failures: list[str] = []
    fixture_count = 0
    ...
    if fixture_count == 0:
        print("[ARG-V3.7.1 LINT WARN: no expected_passport.yaml fixtures found under "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.fixture is not None:
        if not args.fixture.exists():
            print(f"[ARG-V3.7.1 LINT ERROR: fixture not found: {args.fixture}]")
            return 1
