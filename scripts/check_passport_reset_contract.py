#!/usr/bin/env python3
"""Enforce the ARG v3.6.3 passport-reset-boundary co-location contract.
    ...
    for path in sorted(root.rglob("*")):
        ...
        if PROTOCOL_TOKEN not in content:
            ...
            violations.append(
                ...
                f"{PROTOCOL_TOKEN} (co-location required by ARG v3.6.3 contract)"
            )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Enforce the ARG v3.6.3 passport-reset co-location contract: every "
