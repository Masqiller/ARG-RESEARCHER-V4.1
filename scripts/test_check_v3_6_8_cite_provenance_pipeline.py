"""Mutation tests for ARG v3.7.1 Step 3b — Cite-Time Provenance Finalizer.
...
def test_rescind_affordance_clause_missing_fails() -> None:
    """R3 P2-2 closure: the canonical block must mention `/arg-unmark-read`
    so the bidirectional rescind contract survives prose edits.
    """
    with _Snapshot(TARGET):
        text = TARGET.read_text(encoding="utf-8")
        marker = "/arg-unmark-read"
        assert marker in text, "fixture missing /arg-unmark-read mention"
        mutated = text.replace(marker, "/arg-rescind")
        TARGET.write_text(mutated, encoding="utf-8")
        result = _run_lint()
        assert result.returncode == 1, (
            "R3 P2-2: removing /arg-unmark-read must fail the lint"
        )
