#!/usr/bin/env python3
"""ARG v3.7.1 Step 3b — Cite-Time Provenance Finalizer (academic-pipeline mode).
...
_REQUIRED_CLAUSES: list[tuple[str, str, str]] = [
    ...
    # R3 P2-2 closure: R2 closed a P1 specifically about /arg-unmark-read
    ...
    (
        ...
        "/arg-unmark-read",
        "Spec §3.6 line 302 defines /arg-unmark-read as the rescind "
    ...
    (
        ...
        "demotion (e.g. `ok` → `LOW-WARN` after /arg-unmark-read) so "
...
def check_finalizer_subsection(verbose: bool = True) -> int:
    ...
    if not ORCHESTRATOR.exists():
        print(
            "[ARG-V3.7.1 STEP-3B LINT ERROR: orchestrator agent file missing "
    ...
    if failures:
        print(
            "[ARG-V3.7.1 STEP-3B LINT ERROR: Cite-Time Provenance Finalizer "
