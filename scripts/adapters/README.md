# ARG Literature Corpus Adapters

Reference adapters that produce `literature_corpus[]` entries for the ARG Material Passport. Defined by the adapter contract at [`academic-pipeline/references/adapters/overview.md`](../../academic-pipeline/references/adapters/overview.md).
...
Before using the passport in any ARG workflow:
...
`abstract` and `user_notes` can contain publisher-copyrighted material. Before sharing a passport that contains these fields publicly (e.g., in a public repo or over the web), make sure you have the right to publish that text. ARG does not enforce this — it only warns you in the schema description.
