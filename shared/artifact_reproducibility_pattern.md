...
ARG produces research artifacts — draft papers, literature reviews, review reports. A year
from now, you may want to reproduce one. Today's Material Passport (Schema 9) captures WHO
produced the artifact and WHEN, but not WITH WHAT. Which LLM? Which ARG version? Which
...
that ARG-produced artifacts carry their configuration on record — not as a replay mechanism,
...
deterministic schedulers, and cached external data — none of which ARG controls today.
