...
ARG inherits this risk the moment a user publishes an "ARG beats manual" claim. This
pattern defines a mandatory schema for anyone publishing an ARG benchmark comparison.
Reports that don't satisfy the schema aren't "ARG benchmarks" — they're anecdotes.
...
- **`author_independence`** (enum: author-conducted | author-blinded | third-party-conducted):
  ...
  flags the downward-bias risk. Authors know what a good ARG output looks like; their
  human effort is calibrated against that prior. `author-blinded` (authors worked without
  seeing ARG outputs first) is better. `third-party-conducted` eliminates experimenter
  bias entirely.
...
- **`tools_allowed`** (array of strings): "No tools" vs "ChatGPT allowed" is not the same
  comparison as ARG. If the human baseline used no AI assistance while ARG ran full
...
- **`data_access_level_declared`** (enum: raw | redacted | verified_only): If ARG ran
...
- **`skills_used`** (array, minItems 1): A benchmark running `academic-pipeline`
  full-mode is not comparable to one running only `deep-research`. Both are ARG;
...
output came from ARG versus human baseline) is the minimum credible standard for any
...
- **Omitting sample size.** The field is required. There is no valid ARG benchmark report
...
- **Claiming ARG advantage without disclosing cost_usd.** The field is required. A claim
  that "ARG completed this in 4 hours vs 40 human hours" without a dollar figure omits
