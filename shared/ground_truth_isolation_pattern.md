...
Two published cases make this concrete and directly inform the ARG design.
...
ARG's human-in-the-loop pipeline already enforces the spirit of this
...
Every artifact in ARG belongs to one of three layers, and the direction of
...
No ARG skill operates on layer 3 inputs and produces layer 1 or 2 outputs.
...
The isolation pattern is already instantiated across the ARG codebase through
...
**Not a runtime permission system.** Nothing in ARG enforces isolation at
...
**Not a benchmark protocol.** All current ARG skills are `task_type:
open-ended` because ARG targets humanities research, higher-education quality
...
infrastructure for running ARG outputs through automatic scoring against a
