...
Most publishers use a whitespace-equivalent algorithm in practice but state different conventions in their author guidelines. The 3–5% buffer rule (below) makes ARG pipelines robust to the convention divergence without per-publisher adapters.

### Common publisher families

| Publisher / venue | Stated algorithm | Practical effect |
|---|---|---|
| Springer / Nature | whitespace-equivalent for abstract, no explicit definition | matches ARG canonical |
| IEEE | "approximately N words" — whitespace-equivalent in their submission tool | matches ARG canonical |
| ACM | whitespace-equivalent in CCS submission | matches ARG canonical |
| Elsevier (Cell, Lancet families) | whitespace-equivalent in EM submission | matches ARG canonical |
| Wiley | whitespace-equivalent | matches ARG canonical |
| arXiv | no hard cap on abstract; advisory ~250 words | guideline only |
| ICLR / NeurIPS / ACL workshop tracks | hyphenated-as-1 in their LaTeX `\abstract{}` counter | matches ARG canonical |
...
If the publisher's algorithm is ambiguous, default to ARG canonical (whitespace-split) plus the 3–5% buffer.

---

## The 3–5% buffer rule

ARG pipelines reserve **3–5% below the publisher's stated hard cap** as buffer. For a 250-word abstract:
...
Mandarin, Japanese, Korean, Thai, and Lao do not delimit words with whitespace. ARG canonical algorithm produces meaningless totals on these texts.
...
- ARG feedback memory `feedback_word_count_convention_mismatch.md` — original empirical observation of whitespace-split vs. hyphenated-as-1 differential (~12 words on a 250-word abstract).
